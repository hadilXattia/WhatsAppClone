...
export default function ListProfils(props) {
  ...
  useEffect(() => {
    const chatsRef = ref_chats.child(currentUserid);
    chatsRef.on("value", (snapshot) => {
      let messages = {};
      snapshot.forEach((child) => {
        const msgArray = Object.values(child.val() || {});
        if (msgArray.length > 0) {
          messages[child.key] = msgArray[msgArray.length - 1].message || "";
        }
      });
    ...
    return () => {
      ref_chats.child(currentUserid).off("value");
    };
  }, []);
