...
const Chat = (props) => {
  ...
  useEffect(() => {
    ...
    const listener = refSecondUser.on('value', (snapshot) => {
      ...
      if (userData) {
        ...
        setSecondUserPhoto(userData.image || null);
  ...
  const lastMyMessageIndex = useMemo(() => {
    return [...messages]
      .map((msg, idx) => (msg.type === 'message' && msg.senderId === currentid ? idx : null))
      .filter((idx) => idx !== null)
      .pop();
  }, [messages]);

  const sendMessage = (content) => {
    const ref_messages = ref_listDisscussion.child(discussionId).child('messages');
