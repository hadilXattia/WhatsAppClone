...
export default function GroupChat({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    typingRef.on('value', snapshot => {
     ...
     .map(([uid]) => usernames[uid]?.name || 'Someone');
  ...
  const renderItem = ({ item }) => {
    ...
    const senderInfo = usernames[item.sender] || {};
    const senderImage = senderInfo.image;
    ...
    return (
      <View style={[
        ...
        {!isMe && (
          <Image
            source={senderImage ? { uri: senderImage } : require('../assets/icons/user.png')}
            style={styles.avatar}
          />
        )}
        <View style={[
          ...
          <View style={styles.metaRow}>
            <Text style={styles.time}>
              {senderInfo.name || item.sender} • {formatTime(item.timestamp)}
            </Text>
            {isMe && item.seenBy &&
